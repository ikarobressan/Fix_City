...
class SignupScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // WillPopScope previne comportamentos indesejados quando o usuário pressiona o botão "voltar" do dispositivo.
    return WillPopScope(
      // Função que é chamada quando o usuário tenta voltar.
      onWillPop: () async {
        // Se puder voltar para a tela anterior, ele executa a ação.
        if (Navigator.of(context).canPop()) {
          Navigator.of(context).pop();
          // Já lidamos com a ação de voltar.
          return false;
        }
        // Se não puder voltar (por exemplo, se esta for a primeira tela), não faz nada.
        return false;
      },
      child: SafeArea(
